...
if __name__ == "__main__":
    ...
    if training_args.overwrite_output_dir or not os.path.exists(training_args.output_dir):
        training_args.output_dir = training_args.output_dir + "-lorar" + f"-{custom_args.lora_r}" + f"-{model_name}" + "-gradient" + str(training_args.gradient_accumulation_steps) + "-time" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + "-localrank" + local_rank
    ...
    swanlab_callback = SwanLabCallback(project="CHA_LoRA_pretrain", experiment_name=training_args.run_name, config=vars(training_args))
    training_args.report_to = []

    # Logging stuff
    send_example_telemetry("run_CHA_LoRA_pretrain", model_args, data_args)
